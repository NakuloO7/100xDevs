...
const client = new Client({
  connectionString:
    "postgresql://test_owner:UEpMlTZ8R0xQ@ep-shiny-wildflower-a5kgk6yn.us-east-2.aws.neon.tech/test?sslmode=require",
});
...
const insertEmployee = async (
  ...
  try {
    await client.connect();
    const inserQuery =
      "INSERT INTO employee (username, email, password) VALUES ($1, $2, $3) RETURNING id;";
...
const insertAddress = async (userId : number, 
    city : string, 
    country : string,
    street : string,
    pincode : string
...
const insertData = async ()=>{
    const userId = await insertEmployee("Nakul", "Nakul@gmail.com", "Nak432");
    if(userId){
        await insertAddress(userId,"Rahuri", "India", "Nagar", "431037");
    }
}
